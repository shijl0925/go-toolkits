...
// NewAssociationManager 创建关系管理器
func NewAssociationManager[T any, R any](owner T, fieldName string) AssociationManager[T, R] {
...
func (m *GenericAssociationManager[T, R]) Add(relations ...R) error {
	db := globalDb.Model(&m.owner).Association(m.fieldName)
	if err := db.Error; err != nil {
		return err
	}
	return db.Append(relations)
}
...
func (m *GenericAssociationManager[T, R]) Set(relations []R) error {
	...
	return db.Replace(relations)
}

// Count 统计关联数量
func (m *GenericAssociationManager[T, R]) Count() (int64, error) {
	db := globalDb.Model(&m.owner).Distinct().Association(m.fieldName)
	if err := db.Error; err != nil {
		return 0, err
	}
	return db.Count(), nil
...
func (m *GenericAssociationManager[T, R]) All() ([]R, error) {
	...
	var relations []R
	if err := db.Find(&relations); err != nil {
		return nil, err
	}
